...
export function KindergartenOralActivity({ profile, kind, onBack }: { profile: Profile; kind: OralKind; onBack(): void }) {
  ...
  if (finished) return <section className="page kindergarten-finish"><DevoirosAvatar id={profile.devoiros} /><h1>Bravo !</h1><p>{score} bonne{score > 1 ? "s" : ""} réponse{score > 1 ? "s" : ""}.</p><button className="primary-button" onClick={onBack}>Retour aux activités</button></section>;

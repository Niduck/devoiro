...
export function KindergartenOralActivity({ profile, kind, onBack, onComplete }: { profile: Profile; kind: OralKind; onBack(): void; onComplete?(score: number): void }) {
  ...
  if (finished) return <section className="page kindergarten-finish"><DevoirosAvatar id={profile.devoiros} /><h1>Bravo !</h1><p>{score} bonne{score > 1 ? "s" : ""} réponse{score > 1 ? "s" : ""}.</p><button className="primary-button" onClick={() => onComplete ? onComplete(score) : onBack()}>{onComplete ? "Étape suivante →" : "Retour aux activités"}</button></section>;
...
export function AlphabetSongActivity({ profile, onBack, onComplete }: { profile: Profile; onBack(): void; onComplete?(): void }) {
  const [started, setStarted] = useState(false);
